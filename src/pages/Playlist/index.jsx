...
const Playlist = () => {
	...
	const getPlaylistPodcasts = async (id) => {
		try {
			setIsFetching(true);
	...
	const handleRemovePodcast = async (podcastId) => {
		...
		const payload = { playlistId: playlist._id, podcastId };
		const filterPodcasts = originalPodcasts.filter((podcast) => podcast._id !== podcastId);
		setPodcasts(filterPodcasts);
		const res = await removePodcastFromPlaylist(payload, dispatch);
		!res && setPodcasts(originalPodcasts);
	};

	useEffect(() => {
		getPlaylistPodcasts(id);
	}, [id]);
